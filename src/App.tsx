...
function HabitCard({ habit, weekDates, onComplete, onUndo, onEdit, onReset, onArchive }: { habit: Habit; weekDates: Date[]; onComplete: (habit: Habit, key: string) => void; onUndo: (habit: Habit, key: string) => void; onEdit: (habit: Habit) => void; onReset: (habit: Habit) => void; onArchive: () => void }) {
  ...
  const goalReachedElsewhere = weeklyDone && !doneToday
  return <article className="habit-card"><div className="habit-card-main"><div className="habit-title-row"><div><span className={`frequency-tag ${habit.frequency}`}>{habit.frequency === 'daily' ? 'Daily' : 'Weekly'}</span><h3>{habit.name}</h3></div><div className="streak"><strong>{streak}</strong><span>{habit.frequency === 'daily' ? 'day streak' : 'week streak'}</span></div></div>{habit.notes && <p className="notes">{habit.notes}</p>}<div className="card-footer"><div className="mini-progress"><div><span>{habit.frequency === 'weekly' ? 'This week' : 'Weekly rhythm'}</span><b>{progress} / {target}</b></div><div className="progress-track"><i style={{ width: `${Math.min(100, (progress / target) * 100)}%` }} /></div></div><button className={`complete-button ${doneToday ? 'complete' : ''}`} disabled={goalReachedElsewhere} onClick={() => doneToday ? onUndo(habit, today) : onComplete(habit, today)} aria-label={doneToday ? `Undo ${habit.name} completion today` : goalReachedElsewhere ? `${habit.name} weekly goal reached` : `Complete ${habit.name} today`}>{weeklyDone ? '✓ Goal reached' : doneToday ? `✓ ${todayCount} done today` : 'Mark complete'}</button></div></div><div className="card-actions"><button onClick={() => onEdit(habit)}>Edit</button><button onClick={() => onReset(habit)}>Reset</button><button onClick={onArchive}>{habit.archived ? 'Restore' : 'Archive'}</button></div></article>
}
